...
linkPlayerLeftBullet = './data/bullet/bullet left.png'
linkPlayerRightBullet = './data/bullet/bullet right.png'
...
musicBackground = mixer.Sound('./data/sound/Victory.wav')
musicMenu = mixer.Sound('./data/sound/musicmenu.wav')
musicTheme = mixer.Sound('./data/sound/musictheme.wav')
...
def pause():  # store
    ...
    while paused:
        ...
        clusterLaser_coin = 500
        semicircularLaser_coin = 2000
...
def main_menu():
    ...
    menu_run = True
    while menu_run:
        musicMenu.set_volume(0.1)
        musicMenu.play(0) # lặp 0 lần
        screen.blit(background, (0, 0))
...
def tutorial():
    ...
    while tutor_run:
        ...
        show_texts(xScreen / 2 + 30, 245, ": Shoot", 25)

        screen.blit(Esc_key, (135, 275))
        show_texts(xScreen / 2 + 30, 295, ": Back (can use ingame)", 25)

        screen.blit(Exit_key, (135, 325))
        show_texts(xScreen / 2 + 30, 345, ": Exit game without save", 25)

        show_texts_middle(380, "Space store", 40)
        show_texts(20, 430, "- You can buy items in space store by using coin from game.", 20)
        show_texts(20, 460, "- Destroy enemy ship will get you coins.", 20)
        show_texts(20, 490, "- Different enemy earn different coins.", 20)
        show_texts(20, 520, "- Coins will be reset if you lost.", 20)
        show_texts(20, 550, "- Space store is in paused screen.", 20)

        show_texts(20, 600, "- You can pause the game by press Esc while playing.", 20)
        show_texts(20, 630, "- Lives will be reset to when level up.", 20)
        show_texts(20, 660, "- Press Close (Exit) will close the game immediately. ", 20)
